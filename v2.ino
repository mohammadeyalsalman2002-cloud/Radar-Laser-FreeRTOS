...
// ------------ Timer2 Servo Driver (بدون Servo.h) -------------
// Timer2 CTC, prescaler 128 → tick = 8µs
// إطار 20ms = 2500 tick
...
// مخارج السيرفو عبر PORTD (D6=PD6, D7=PD7)
...
void servo_driver_init(){
  ...
  // Timer2: CTC + prescaler 128 + enable ISR
  TCCR2A = (1<<WGM21);
  TCCR2B = T2_PRESC_128;
  TCNT2  = 0;
  OCR2A  = 10;
...
long readEchoUS() {
  ...
  digitalWrite(trigPin, LOW);
  return pulseIn(echoPin, HIGH, MAX_TIMEOUT); // Blocking, لكن بسيطة
}
...
void TaskScan(void *pvParameters){
  bool forward=true;
  for(;;){
    // حرّك سيرفو الماسح (بدل scanner.write)
  ...
  for(;;){
    if(objectTargetAngle>=0){
      ...
      if(diff!=0){
        ...
        // بدّل laserServo.write(...) بـ سواقة Timer2
...
void setup(){
  ...
  // استبدال attach للسيرفو ببدء السواقة على Timer2
  servo_driver_init();

  Serial.begin(115200);
  Serial.println(F("BOOT"));

  // قلّل أحجام الstack لتفادي استنزاف الRAM
